...
public class Challenge2Test
{
	...
	public void Example_Machine_1_Should_Produce_Expected_Values()
	{
		...
		var input = new PuzzleMachine(
			new Coord(94, 34),
			new Coord(22, 67),
			new Coord(8400 + Offset, 5400 + Offset)
		);
	...
	public void Example_Machine_2_Should_Produce_Expected_Values()
	{
		...
		var input = new PuzzleMachine(
			new Coord(26, 66),
			new Coord(67, 21),
			new Coord(12748 + Offset, 12176 + Offset)
		);
		...
		var totalButtonAOffsetX = actualButtonPresses.Value.ButtonAPresses * input.ButtonAOffset.X;
		var totalButtonAOffsetY = actualButtonPresses.Value.ButtonAPresses * input.ButtonAOffset.Y;

		var totalButtonBOffsetX = actualButtonPresses.Value.ButtonBPresses * input.ButtonBOffset.X;
		var totalButtonBOffsetY = actualButtonPresses.Value.ButtonBPresses * input.ButtonBOffset.Y;

		Assert.Equal(input.PrizeLocation.X, totalButtonAOffsetX + totalButtonBOffsetX);
		Assert.Equal(input.PrizeLocation.Y, totalButtonAOffsetY + totalButtonBOffsetY);
	}
	...
	public void FindMinimumCostToSolveMachine_Example_Machine_3_Should_Produce_Expected_Value()
	{
		...
		var input = new PuzzleMachine(
			new Coord(17, 86),
			new Coord(84, 37),
			new Coord(7870 + Offset, 6450 + Offset)
		);
	...
	public void FindMinimumCostToSolveMachine_Example_Machine_4_Should_Produce_Expected_Value()
	{
		...
		var input = new PuzzleMachine (
			new Coord(69, 23),
			new Coord(27, 71),
			new Coord(18641 + Offset, 10279 + Offset)
		);
...
[TestSubject(typeof(Challenge2))]
public class Challenge2OriginalValuesTest
{
	[Fact]
